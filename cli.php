...
reqemail:
echo "$WH [$GR+$WH] Threads ($YL Max 100 $WH) ($YL Recommended 10-30 $WH) $GR>> $BL";
$reqemail = trim(fgets(STDIN));
$reqemail = (empty($reqemail) || !is_numeric($reqemail) || $reqemail <= 0) ? 50 : $reqemail;
if ($reqemail > 100) {
    echo PHP_EOL . PHP_EOL . "$WH [$YL!$WH] $RD MAX 100$WH [$YL!$WH]$DEF" . PHP_EOL . PHP_EOL;
    goto reqemail;
}
...
$rollingCurl->setCallback(function (\RollingCurl\Request $request, \RollingCurl\RollingCurl $rollingCurl) use (&$results) {
    ...
    if (strpos($x, '"status":"live"')) {
        ...
        echo "[$RD$no$DEF/$GR$total$DEF][$CY$jam$DEF]$GR LIVE$DEF =>$BL $list$DEF | [$YL MSG$DEF: $WH$msg$DEF ] | BY$CY DARKXCODE$DEF (V3.5)" . PHP_EOL;
    } else if (strpos($x, '"msg":"Incorrect APIkey!"')) {

        echo "[$RD$no$DEF/$GR$total$DEF][$CY$jam$DEF]$RD DIE$DEF =>$BL $list$DEF | [$YL MSG$DEF:$MG Incorrect APIkey!$DEF ] | BY$CY DARKXCODE$DEF (V3.5)" . PHP_EOL;

    } else if (strpos($x, '"status":"die"')) {
        $die++;
        save_file("result/die.txt", "$list");
        echo "[$RD$no$DEF/$GR$total$DEF][$CY$jam$DEF]$RD DIE$DEF =>$BL $list$DEF | [$YL MSG$DEF: $MG$msg$DEF ] | BY$CY DARKXCODE$DEF (V3.5)" . PHP_EOL;
    } else if (strpos($x, '"status":"unknown"')) {
        $limit++;
        save_file("result/limit.txt", "$list");
        echo "[$RD$no$DEF/$GR$total$DEF][$CY$jam$DEF]$CY LIMIT$DEF =>$BL $list$DEF | [$YL MSG$DEF: $MG$msg$DEF ] | BY$CY DARKXCODE$DEF (V3.5)" . PHP_EOL;
    } else if ($x == "") {
        $rto++;
        save_file("result/RTO.txt", "$list");
        echo "[$RD$no$DEF/$GR$total$DEF][$CY$jam$DEF]$DEF TIMEOUT$DEF =>$BL $list$DEF | [$YL MSG$DEF:$MG REQUEST TIMEOUT!$DEF ] | BY$CY DARKXCODE$DEF (V3.5)" . PHP_EOL;
        ...
        //save_file("result/response.txt", "$x");

        echo "[$RD$no$DEF/$GR$total$DEF][$CY$jam$DEF]$YL UNKNOWN$DEF =>$BL $list$DEF | BY$CY DARKXCODE$DEF (V3.5)" . PHP_EOL;
    }
